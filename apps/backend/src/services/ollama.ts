...
If you don't need to call a function, respond normally with text.`;
    ...
    async continueWithFunctionResult(
        ...
        logger.debug(`Ollama continue with ${functionResults.length} function results`);

        // Add function results to history
        const functionResultsText = functionResults.map(r =>
            `Function ${r.functionResponse.name} returned: ${JSON.stringify(r.functionResponse.response)}`
        ).join('\n');

        const messages = [
            ...chat.history,
            { role: 'user', content: `Function results:\n${functionResultsText}\n\nPlease provide a helpful response based on these results.` }
        ];
    ...
    private buildMessages(history: any[], message: string, tools: Tool[]) {
        ...
        for (const item of history) {
            if (item.role && item.content) {
                ...
                // Gemini format
                const content = item.parts.map((p: any) => p.text || JSON.stringify(p)).join(' ');
                messages.push({ role: item.role === 'model' ? 'assistant' : item.role, content });
            }
        }
    ...
    private formatToolsForPrompt(tools: Tool[]): string {
        ...
        const declarations = tools.flatMap(t =>
            (t as any).functionDeclarations || []
        );

        return declarations.map((fn: any) => {
            const params = fn.parameters?.properties
                ? Object.entries(fn.parameters.properties).map(([name, schema]: [string, any]) =>
    ...
    private parseFunctionCall(content: string): { name: string; args: any } | null {
        try {
            ...
            logger.debug(`Failed to parse function call from: ${content.substring(0, 200)}`);
        }
        return null;
    }
}
