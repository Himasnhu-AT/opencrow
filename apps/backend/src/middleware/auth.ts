...
export function authMiddleware(req: Request, res: Response, next: NextFunction) {
    ...
    return res.status(401).json({ error: 'Authorization required. Provide Bearer token or X-API-Key.' });
}

/**
 * Middleware specifically for chat routes that allows both JWT and API key
 */
export function chatAuthMiddleware(req: Request, res: Response, next: NextFunction) {
    const authHeader = req.headers.authorization;
    const apiKey = req.headers['x-api-key'] as string;
...
async function validateApiKey(apiKey: string, req: Request, res: Response, next: NextFunction) {
    try {
