...
export class OpenAPIParser {
    ...
    for (const [path, methods] of Object.entries(paths)) {
      for (const [method, operation] of Object.entries(methods as any)) {
        if (["get", "post", "put", "patch", "delete"].includes(method)) {
          tools.push(this.createFunctionDeclaration(path, method, operation));
        }
      }
    }
  ...
  /**
   * Extract endpoints for display in the frontend
   */
  extractEndpoints(openApiSpec: any): Array<{
    operationId: string;
    method: string;
    path: string;
    summary: string;
  ...
  private extractParameters(operation: any) {
    ...
    if (operation.requestBody?.content?.["application/json"]?.schema) {
      ...
      if (schema.properties) {
        Object.assign(
          properties,
          this.convertSchemaProperties(schema.properties),
        );
